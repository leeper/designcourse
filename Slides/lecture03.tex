...
\begin{tikzpicture}
    ...
    \node[below] at (2.5,-0.75) {\small Intension (More cases)};
	\node[below] at (0.5,0) {\footnotesize Low};
	\node[below] at (5,0) {\footnotesize High};
	
	% y-axis
    \draw[->] (origin) -- (0,5) node[left] (yaxis) {};
	\node[left, text width=5cm, align=center] at (0,2.5) {\small Extension\\(More attributes)};
...
% If we increase necessary attributes, we have fewer cases of that concepta and it is more specific
